...
class Ship(GImage):
    ...
    def __init__(self,x,y,width,height,source):
        super().__init__(x=x,y=y,width=SHIP_WIDTH,height=SHIP_HEIGHT,source=SHIP_IMAGE)
    ...
    def collides(self,bolt):
        ...
        if bolt._velocity>0:
            return False
        l = bolt.x - BOLT_WIDTH/2
        r = bolt.x + BOLT_WIDTH/2
        u = bolt.y + BOLT_HEIGHT/2
        d = bolt.y - BOLT_HEIGHT/2
        if self.contains((l,u)) or self.contains((r,u)) \
        or self.contains((r,d)) or self.contains((l,d)):
            return True
...
class Alien(GImage):
    ...
    dimensions. These dimensions are all specified in consts.pse
    You also MIGHT want to add code to detect a collision with a bolt. We
    do not require this.  You could put this method in Wave if you wanted to.
    But the advantage of putting it here is that Ships and Aliens collide
    with different bolts.  Ships collide with Alien bolts, not Ship bolts.
    ...
    def collides(self,bolt):
        ...
        Precondition: bolt is of class Bolt
        """
        assert isinstance(bolt,GRectangle) or isinstance(bolt,list)
        if bolt == []:
            return False
...
class Bolt(GRectangle):
    ...
    def __init__(self,x,y,velo):
        """Creates the bolt at point x,y with velocity velo"""
        self.setVelocity(velo)
        super().__init__(x=x,y=y,width=BOLT_WIDTH,height=BOLT_HEIGHT,fillcolor='red',linecolor='red')

    # GETTERS AND SETTERS (ONLY ADD IF YOU NEED THEM)

    def setVelocity(self,velo):
        """
        Sets velocity of the bolt
        parameter: velo is the velocity of the bolt in the y direction
        precondition: velo is an int or float
        """
        assert isinstance(velo,int) or isinstance(velo,float)
        self._velocity = velo
