...
class Invaders(GameApp):
    ...
    #
    # Attribute _second_text: a secondary label for the active text
    # Invariant: _second_text is a GLabel object, or None if there is no message to
    # display. It is onl None if _state is STATE_ACTIVE.
    #
    # Attribute _num_wave: the wave number we are on, impacts difficulty
    # Invariant _num_wave is an int between [0,infinity]
    #
    # Attribute _hearts: the lives the player has going into the wave
    # Invariant _hearts is an int between [1,infinity]
    #
    ...
    def start(self):
        ...
        self._text=GLabel(text="Alien " + \
        "Invaders\nJack Lehman & Arefa Rahman\nPress 's' to Play",\
        font_name="RetroGame.ttf",font_size=35, \
    ...
    def update(self,dt):
        ...
        if self.input.is_key_pressed('s') and not self._state == STATE_ACTIVE \
        and not self._state == STATE_PAUSED and not self._state == STATE_CONTINUE\
        and not self._state == STATE_COMPLETE:
            self._state=STATE_NEWWAVE
        if self._state==STATE_NEWWAVE:
            self._wave= Wave(self._num_wave, self._hearts)
            self._state=STATE_ACTIVE
        if self._state==STATE_ACTIVE:
            self._wave.update(self.input,dt)
            self.drawActive()
            if self._wave.isGameOver():
                self._state = STATE_COMPLETE
            elif self.input.is_key_pressed('spacebar') or self._wave.getCollided():
    ...
    def draw(self):
        ...
        if self._state==STATE_COMPLETE:
            self._wave.draw(self.view)
            self._text.draw(self.view)
            self._second_text.draw(self.view)
        if self._state == STATE_PAUSED:
            self._text.draw(self.view)
            self._second_text.draw(self.view)
            self._wave.draw(self.view)
    ...
    def drawEnd(self):
        """
        Draws the end of the wave.

        What it draws depends on if the player completed the wave
        or lost the game.
        """
        if self._wave.checkWin():
            self._text=GLabel(text="Wave Complete!\nPress S to continue",\
            font_name="RetroGame.ttf",font_size=50, linecolor='magenta')
            self._text.x=GAME_WIDTH/2
            self._text.y= GAME_HEIGHT/2
        else:
            self._text=GLabel(text="You Lose\nPress S to play again",\
            font_name="RetroGame.ttf",font_size=50, linecolor = 'red')
            self._text.x=GAME_WIDTH/2
            self._text.y= GAME_HEIGHT/2
        self._second_text=GLabel(text="Lives: " + str(self._wave.getLives()) + \
        "\nWave: " + str(self._num_wave+1),\
        font_name="RetroGame.ttf",font_size=20, linecolor='navy')
        self._second_text.x=GAME_WIDTH - 2*ALIEN_WIDTH
        self._second_text.y= GAME_HEIGHT - 2*ALIEN_H_SEP

    def drawPause(self):
        """
        Draws the pause screen after the ship was hit or if the spacebar was pressed
        """
        if self._wave.getCollided():
            self._text=GLabel(text="Collision Detected!\nPress 'S' to continue",\
            font_name="RetroGame.ttf",font_size=40, linecolor='black')
            self._text.x=GAME_WIDTH/2
            self._text.y= GAME_HEIGHT/2
        else:
    ...
    def drawActive(self):
        """
        Draws the number of lives and current wave number on the top right
        during the active wave
        """
        self._text=GLabel(text="Lives: " + str(self._wave.getLives()) + \
        "\nWave: " + str(self._num_wave+1),\
        font_name="RetroGame.ttf",font_size=20, linecolor='navy')
        self._text.x=GAME_WIDTH - 2*ALIEN_WIDTH
        self._text.y= GAME_HEIGHT - 2*ALIEN_H_SEP

    def endGame(self):
        """
        Helper function for the STATE_COMPLETE, goes to next wave if player
        completed the current wave and presses 's,' starts from the beginning
        if the player presses s and lost the game
        """
        if self.input.is_key_pressed('s') and self._wave.checkWin():
            self._num_wave = self._num_wave + 1
            self._hearts = self._wave.getLives() + 1
            self._state = STATE_NEWWAVE
